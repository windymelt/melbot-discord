...
object Testing extends ResourceApp {
  ...
  class PrintlnProcessor[Handler <: GatewayHandler[IO]](
      console: Console[IO],
  ...
  class MyCommands(requests: Requests[IO, Any], components: Components[IO])
    ...
    val infoCommand: SlashCommand[IO, Unit] =
      SlashCommand.command("info", "Get some info") { invocation =>
        sendMessage(
    ...
    val infoCommandAsync: SlashCommand[IO, Unit] =
      SlashCommand.command("infoasync", "Get some info async") { invocation =>
        async(invocation) { implicit a =>
          sendAsyncMessage(
            CreateFollowupMessageBody.ofContent(
              "Hello from AckCord 2.0 async application commands"
            )
          )
        }
      }
            ...
            .installEventListener { case ready: GatewayDispatchEvent.Ready =>
              InteractionsRegistrar
                .createGuildCommands[IO](
                  ...
                  GuildId("1174392787228184577": String),
                  settings.requests,
                  replaceAll = true,
            ...
            new StringCommandProcessor(
              Map(
                    ...
                    .runRequest(
                      ChannelRequests.createMessage(
                        ...
                        ChannelRequests.CreateMessageBody
                          .ofContent("Hello from AckCord 2.0")
                      )
                    )
                    .void
                }),
                "!ratelimitTest " -> ((content, channelId) => {
                  List
                    .tabulate(
                      content.substring("!ratelimitTest ".length).toInt
                    )(i =>
                      ChannelRequests.createMessage(
                        channelId,
                        ChannelRequests.CreateMessageBody
                          .ofContent(s"Ratelimit message ${i + 1}")
                      )
                    )
                ...
                "!componentTest" -> ((_, channelId) => {
                  ...
                  for {
                    btn <- components.buttons.make(label = Some("Press me")) {
                      _ =>
                        new InteractionHandlerOps[IO] {
                          ...
                          val res: IO[
                            HighInteractionResponse.AsyncMessageable[IO]
                          ] =
                            IO.pure(
                              sendMessage(
                                MessageData.ofContent("You pressed the button")
                              )
                            )
                        }.res
                    }
